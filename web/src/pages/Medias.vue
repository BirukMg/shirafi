    ...
    <section class="container">
        <v-app style="background-color: #fff">
            <v-toolbar flat color="white">
                <v-toolbar-title>Medias</v-toolbar-title>
                <v-divider class="mx-2" inset vertical></v-divider>
                <v-spacer></v-spacer>
                <mdb-btn size="sm" outline="primary" @click="addMediaModal = true"><mdb-icon icon="plus" class="mr-1"/> New Media</mdb-btn>
            </v-toolbar>
            <v-data-table :headers="headers" :items="medias" class="elevation-1">
                <template slot="items" slot-scope="props">
                    <td class="text-xs-left" style="width: 1px">{{medias.indexOf(props.item) + 1}}</td>
                    <td class="text-xs-left" style="width: 1px">
                        <img src="../assets/images/logo.jpg" class="post-img" alt="">
                    </td>
                    <td class="text-xs-left" style="width: 1px">{{props.item.name}}</td>
            ...
            <mdb-modal v-if="deleteModal" @close="deleteModal = false">
                ...
                <mdb-modal-body>
                    ...
                    <h5 class="font-weight-bold text-danger">Delete {{deleteData.name}} ??</h5>
                </mdb-modal-body>
                <mdb-modal-footer>
                    <section style="margin-right: 7%">
                        <mdb-btn color="danger" size="sm" @click="deleteMedia">Delete account only</mdb-btn>
            ...
            <mdb-modal size="lg" v-if="editModal" @close="editModal = false">
                ...
                <mdb-modal-body>
                  ...
                  <mdb-input type="text" label="Name" v-model="editData.name" />
                ...
                <mdb-modal-footer>
                    <mdb-btn size="sm" outline="success" @click="editMedia">Save changes</mdb-btn>
                </mdb-modal-footer>
            </mdb-modal>


            <mdb-modal v-if="addMediaModal" @close="addMediaModal = false">
                <mdb-modal-header>
                    <mdb-modal-title>New Media</mdb-modal-title>
                </mdb-modal-header>
                <mdb-modal-body>
                    <h5 :class="addmsg.styles">{{addmsg.title}}</h5>
                    <mdb-input type="text" label="Name" v-model="addData.name"/>
                ...
                <mdb-modal-footer>
                    <mdb-btn size="sm" outline="success" @click="addMedia">Save</mdb-btn>
                </mdb-modal-footer>
            </mdb-modal>
        </v-app>
    </section>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                { text: '#',align: 'left',value: 'medias_no'},
                { text: 'Profile Picture',align: 'left',value: 'profile_picture'},
                { text: 'Media name',align: 'left', value: 'media_name' },
    ...
    methods: {
        init(){
            axios.get('http://' + this.$store.state.server_ip + ':3000/api/media')
            .then(res => {
        ...
        addMedia(){
            ...
            if (this.passed) {
                this.addData.username = this.defaultUsername(this.addData.name)
                ...
                axios.post('http://' + this.$store.state.server_ip + ':3000/api/shirafi_users',{
                    ...
                    axios.post('http://' + this.$store.state.server_ip + ':3000/api/shirafi_users/'+ res.data.id +'/media',this.addData)
                    .then(res => {
                        this.init()
                        this.addmsg.title = 'New Media added'
            ...
            if (this.passed) {
                if (this.usernameAvailable) {
                    axios.patch('http://' + this.$store.state.server_ip + ':3000/api/shirafi_users/'+ this.editData.user_id,{
                        ...
                        axios.patch('http://' + this.$store.state.server_ip + ':3000/api/media/'+ this.editData.id,this.editData)
                        .then(res => {
                            this.init()
        ...
        onOffItem(item){
            axios.patch('http://' + this.$store.state.server_ip + ':3000/api/media/'+ item.id,{active: !item.active})
            .then(res => {
                this.init()
            })
        },
        deleteMedia(){
            axios.delete('http://' + this.$store.state.server_ip + ':3000/api/media/'+ this.deleteData.id)
            .then(res => {
                this.init()
